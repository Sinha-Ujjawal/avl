...
class avl:
	...
	def __insertUtil(self, k, data, root):
		...
		return self.__rebalance(root)[0]
	...
	def __successorUtil(self, k, root):
		...
		if k < root._k:
			...
			if root._right != None:
				minn = self.__minUtil(root._right)
				return minn
	...
	def successor(self, k):
		...
		if isinstance(k, (int, float)):
			if not self.isEmpty():
				return self.__successorUtil(k, self._root)
		else:
			raise TypeError('key can only be integers or floating point number')
	...
	def __predecessorUtil(self, k, root):
		...
		if k < root._k:
			...
			if root._left != None:
				maxx = self.__maxUtil(root._left)
				return maxx
		if t == None and root._k < k:
			t = root._k, root._data
		return t
	...
	def predecessor(self, k):
		...
		if isinstance(k, (int, float)):
			if not self.isEmpty():
				return self.__predecessorUtil(k, self._root)
		else:
			raise TypeError('key can only be integers or floating point number')
